...
| **Size**        | ~101 MB | ~126 MB | ~158 MB | ~244 MB | ~1231 MB | ~122 MB |
...
- **Pure Bun Development** → `bun` or `ubun-tu`
...
- **Smallest Size** → `ubun-tu` (158 MB)
- **Most Features** → `bunode` (126 MB)
...
```json
{
  ...
  "image": "ghcr.io/iamvikshan/devcontainer/bunode:latest",
  "customizations": {
    "vscode": {
      "extensions": ["oven.bun-vscode", "oxc.oxc-vscode"]
    }
...
```json
{
  ...
  "image": "ghcr.io/iamvikshan/devcontainer/ubun-tu:latest",
  "customizations": {
    "vscode": {
      "extensions": ["oven.bun-vscode", "oxc.oxc-vscode"]
    }
...
```json
{
  ...
  "image": "ghcr.io/iamvikshan/devcontainer/ubun-tu-node:latest",
  "customizations": {
    "vscode": {
      "extensions": ["oven.bun-vscode", "oxc.oxc-vscode"]
    }
...
1. **ubuntu** (~122 MB) - Tools-focused baseline
2. **ubun-tu** (~158 MB) - Most efficient Ubuntu Bun option
3. **bun** (~101 MB) - Most efficient Bun runtime option
4. **ubun-tu-node** (~244 MB) - Balanced Ubuntu full-stack option
5. **bunode** (~126 MB) - Full-featured but larger

### Compatibility Matrix

| Feature             | bun        | bunode  | ubun-tu    | ubun-tu-node | ubuntu |
| ------------------- | ---------- | ------- | ---------- | ------------ | ------ |
