 ...
 */
function extractExistingRow(
  content: string,
  rowLabelRegex: string,
  headers: string[]
): Record<string, string> {
  const existing: Record<string, string> = {}
  const match = content.match(
    new RegExp(`\\|[ \\t]*${rowLabelRegex}[ \\t]*\\|(.*)`)
  )
 ...
 */
async function updateImageVariantsMd(
  sizes: Record<string, string>,
  versions: Record<string, VersionsMap>
) {
  ...
  if (headerMatch) {
    ...
    // A. Update Size Row
    const existingSizes = extractExistingRow(
      content,
      '\\*\\*Size\\*\\*',
      headers
    )
    ...
    const existingBuns = extractExistingRow(
      content,
      '\\*\\*Bun Version\\*\\*',
      headers
    )
    ...
    const existingNodes = extractExistingRow(
      content,
      '\\*\\*Node\\.js\\*\\*',
      headers
    )
  ...
  for (const [name, size] of Object.entries(sizes)) {
    ...
    if (name === 'ubun-tu') {
      content = content.replace(
        /Smallest size - Only [0-9]+ MB!/g,
        `Smallest size - Only ${size}!`
      )
    } else if (name === 'ubun-tu-node') {
      content = content.replace(
        /Balanced size - Feature-rich at only [0-9]+ MB/g,
        `Balanced size - Feature-rich at only ${size}`
      )
    } else if (name === 'agents') {
      content = content.replace(
        /AI additions - Custom taste workspace at only [0-9]+ MB/g,
        `AI additions - Custom taste workspace at only ${size}`
      )
    }
  }
...
async function main() {
  ...
  if (allowedImages) {
    console.log(
      `Locking documentation updates strictly to: [${Array.from(allowedImages).join(', ')}]`
    )
  }
